...
export class QAService {
  ...
  async evaluateIssue(issueKey: string) {
    ...
    try {
      ...
      this.logger.warn(`Prisma findUnique deferred for ${issueKey}: ${e.message}`);
    }
    ...
    if (hasPR) {
      const allReviews = (issue.pullRequests || []).flatMap((p: any) => p.reviews || []);
      hasApprovedReview = allReviews.some((r: any) => r.state === 'APPROVED');
    }
    ...
    const hasUnitTestAttachment = (issue.attachments || []).some((a: any) => unitTestRegex.test(a.filename));
    const hasUnitTestComment = (issue.comments || []).some((c: any) => unitTestRegex.test(c.body));
    const hasUnitTestCommit = (issue.gitCommits || []).some((c: any) => unitTestRegex.test(c.message));
    ...
    const hasTestCase =
      ...
      (issue.comments || []).some((c: any) => testCaseRegex.test(c.body)) ||
      (issue.attachments || []).some((a: any) => testCaseRegex.test(a.filename));
    ...
    const hasDoc =
      ...
      (issue.attachments || []).some((a: any) => docRegex.test(a.filename)) ||
      (issue.comments || []).some((c: any) => docRegex.test(c.body));
    ...
    try {
      return await this.prisma.qAValidation.create({ data: valObj });
    } catch (e: any) {
      this.logger.warn(`Could not save QAValidation to DB: ${e.message}`);
    }
  ...
  async getValidations(issueKey?: string) {
    try {
      ...
      return await this.prisma.qAValidation.findMany({
        where,
        include: {
          issue: {
            select: {
